...
public class GestureDialogManager {
     ...
     */
    public void showSeekDialog(View parent, int targetPosition, int duration, boolean isLeft) {
        if (mSeekDialog == null) {
            mSeekDialog = new SeekDialog(mActivity, targetPosition, duration, isLeft);
        }
        if (!mSeekDialog.isShowing()) {
            mSeekDialog.setScreenMode(mCurrentScreenMode);
